<script lang="ts">
    ...
    $: reactivePointsScaled = points
        .map((el) => [xScale(el[0]), yScale(el[1]), el[2]])
...
<div class="scatter_plot_container">
    ...
    >
        <g class="y-axis" transform="translate({marginLeft}, 0)">
            <path
                class="domain"
                stroke="black"
                d="M{insetLeft}, {marginTop} V{height}"
            />
    ...
    {#if dotInfo}
        <div
            class="dot_info"
            style="left:{dotInfo[2].clientX + 12}px; top:{dotInfo[2].clientY +
        ...
        >
            <span
                ...
                style="background-color: {colors[
                    points[dotInfo[1]][2]
                ]}; height:{width / 100}px; width:{width / 100}px; "
            />
    ...
    <section
        ...
        style="width:{width / 10}px; font-size: {width / 75}px"
    >
        {#if subsets}
            ...
            {#each subsets as subset, i}
                ...
                >
                    <span
                        class="scatter_legend_span"
                        style="background-color: {colors[i]}; height:{width /
                            50}px; width:{width / 50}px; "
                    />
                    {subset}
                </div>
            {/each}
